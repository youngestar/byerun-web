...
const checkAndAutoSign = async () => {
    ...
    const scheduleNextTick = (delayMs: number = 10 * 60 * 1000) => {
        if (worker) worker.postMessage({ type: 'schedule_tick', delay: delayMs });
    };
    ...
    try {
        ...
        if (e.message === 'AUTO_LOGIN_REQUIRED' || (e.message && e.message.match(/登录|过期|无效|失效/))) {
            ...
            if (relogged) return checkAndAutoSign(); 
        }
        addLog('请求状态接口失败，等待下次轮询', 'error');
        scheduleNextTick(5 * 60 * 1000); // 故障时5分钟后再试
        return;
    }

    if (!clubActivity.value || !clubActivity.value.activityId) {
        scheduleNextTick(15 * 60 * 1000); // 无活动时降低频率
        return;
    }
    ...
    try {
        ...
        if (isNaN(startTs) || isNaN(endTs)) {
            addLog('时间格式解析失败，降级为慢速轮询', 'error');
            scheduleNextTick(10 * 60 * 1000);
            return;
        }
        ...
        // 【动态逻辑判定】
        const timeToStart = startTs - now;
        const timeToEnd = endTs - now;
        let nextDelayMs = 10 * 60 * 1000; // 默认慢速

        // A. 签到逻辑
        if (clubActivity.value.signStatus === '1') {
            // 在签到窗口内 [开始-offset, 开始+offset]
            if (now >= startTs - signInOffsetMs && now <= startTs + signInOffsetMs) {
                await executeRetryTask('1');
                scheduleNextTick(30 * 1000); // 签到后极速刷新状态
                return;
            }
            // 临近窗口（开始前15分钟）
            if (timeToStart > 0 && timeToStart <= 15 * 60 * 1000) {
                nextDelayMs = 3 * 60 * 1000;
            } else if (timeToStart > 0 && timeToStart <= 30 * 60 * 1000) {
                nextDelayMs = 5 * 60 * 1000;
            }
        } 
        // B. 签退逻辑
        else if (clubActivity.value.signInStatus === '1' && clubActivity.value.signStatus === '2') {
            // 在签退窗口内 [结束-offset, 结束+offset]
            if (now >= endTs - signBackOffsetMs && now <= endTs + signBackOffsetMs) {
                await executeRetryTask('2');
                // 签退任务完成后 executeRetryTask 会处理 enabled 状态，这里做最后一次调度
                if (autoSignConfig.value.enabled) scheduleNextTick(60 * 1000);
                return;
            }
            // 临近窗口（结束后15分钟内，或结束前15分钟内）
            if (Math.abs(timeToEnd) <= 15 * 60 * 1000) {
                nextDelayMs = 3 * 60 * 1000;
            } else if (Math.abs(timeToEnd) <= 30 * 60 * 1000) {
                nextDelayMs = 5 * 60 * 1000;
            }
        }
        ...
        addLog('自动逻辑判定异常', 'error');
        scheduleNextTick(5 * 60 * 1000);
    }
};
